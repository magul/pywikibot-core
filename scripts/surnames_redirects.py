    ...
    def treat_page(self):
        ...
        if self.getOption('surnames_last'):
            ...
            if len(name_parts) == 2 and page_t == page_t.title():
                ...
                """page title should be titlecased or at least close to titlecased"""
                if len(SequenceMatcher(None, page_t, page_t.title()).get_matching_blocks()) <= 3:
                    possible_redirects.append(name_parts[1] + ' ' + name_parts[2] + ', ' + name_parts[0])
                    possible_redirects.append(name_parts[2] + ', ' + name_parts[0] + ' ' + name_parts[1])
