...
class HarvestRobot(WikidataBot):
    ...
    def split_list(self, value):
        template_list = pywikibot.list_template_regex.match(value)
        if template_list:
            list_body = template_list.group(1)
            items = list_body.split('*')
        elif pywikibot.list_delimiter_regex.match(value):
            items = pywikibot.list_delimiter_regex.split(value)
        elif ',' in value:
            # FIXME: don't split on commas inside links
            items = value.split(',')
        else:
            items = [value]
