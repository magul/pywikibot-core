...
site = pywikibot.Site()
if not (site.code + '.' + site.family.name) in badWordList or \
   not (site.code + '.' + site.family.name) in logPages:
    pywikibot.error('your language isn\'t supported, see the source code for '
                    'further details')
    sys.exit(1)
ownWordPage = pywikibot.Page(site, badWordList[site.code + '.' +
                                               site.family.name])
try:
    ownWordList = ownWordPage.text
...
def checkPage(title, onlyLastDiff=False):
    if title == logPages[site.code + '.' + site.family.name]:
        return
    ...
    try:
        ...
        if onlyLastDiff:
            try:
                oldver = page.getOldVersion(page.previous_revision_id())
            except IndexError:
    ...
    for badWord in ownWordList:
        if (' ' + badWord + ' ') in text:
            ...
            report = True
    if report:
        logPage = pywikibot.Page(site, logPages[site.code + '.' +
                                                site.family.name])
        try:
            log = logPage.get()
        except:
