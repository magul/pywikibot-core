# -*- coding: utf-8 -*-
"""
Dynamic Text Substitutions Robot.
...
Look at
http://de.wikipedia.org/wiki/Benutzer:DrTrigon/Benutzer:DrTrigonBot/config.css
...
bot_config = {
    # unicode values
    'TemplateName': 'User:DrTrigonBot/Subster',  # or 'template' for 'Flagged Revisions'
    'data_PropertyId': '370',  # default: Sandbox-String (P370)

    'ErrorTemplate': "'''SubsterBot Exception in \"%s\" (%s)'''\n<pre>%s</pre>",
    'VerboseMessage': '<noinclude>\n----\n%s\n</noinclude>',  # DRTRIGON-116, DRTRIGON-132
    'data_VerboseMessage': '<onlyinclude>{{#switch: {{{1|}}}\n'
                           '|          error = %(error)s\n'
                           '|error-traceback = %(error-traceback)s\n'
                           '}}</onlyinclude>',

    # important to use a '.css' page here, since it HAS TO BE protected to
    # prevent malicious code injection !
    'ConfCSSpostproc': 'User:DrTrigon/DrTrigonBot/subster-postproc.css',
    'ConfCSSconfig': 'User:DrTrigon/DrTrigonBot/subster-config.css',

    'CodeTemplate': '\n%s(DATA, *args)\n',
    'CRONMaxDelay': 1 * 24 * 60 * 60,       # bot runs daily

    # regex values
    'var_regex_str': '<!--SUBSTER-%(var1)s-->%(cont)s<!--SUBSTER-%(var2)s-->',
...
class SubsterBasicBot(Bot):
    ...
    def save(self, page, text, comment=None, **kwargs):
        ...
        for i in range(3):
            try:
                ...
                pywikibot.output(
                    '\03{lightblue}Cannot change %s because of spam blacklist '
    ...
    def loadTemplates(self, page, template, default={}):
        ...
        @Return: Return a list of dict with the templates parameters found.
        """
        self._content = page.text  # '' if not existing page
    ...
    def loadJobQueue(self, page, queue_security, reset=True):
        ...
        @return: Return a list of jobs. This list may be empty.
        """
        try:
...
class SubsterBot(SubsterBasicBot):
    """Bot to substitute tags within wiki page content.

    Bot which does substitutions of tags within wiki page content
    using external or other wiki text data. Like dynamic text updating.
    """
    ...
    def __init__(self, **kwargs):
        """Initializer of SubsterBot(), initialize needed vars."""
        pywikibot.output('\03{lightgreen}* Initialization of bot:\03{default}')

        super(SubsterBot, self).__init__(**kwargs)
        ...
        if hasattr(time, "tzset"):
            ...
            pywikibot.warning(
                'This operating system has NO SUPPORT for setting TimeZone by '
                'code! Before running this script, please set the TimeZone '
                'manually to one approriate for use with the Wikipedia '
                'language and region you intend to.')

        # init constants
        self._bot_config = bot_config
        self._bot_config['BotName'] = self.site.username()
        ...
        if self._ConfCSSconfigPage.exists():
            global bot_config_wiki  # py3 compatibility
            bot_config_wiki = {}  # F821
            exec(self._ConfCSSconfigPage.get())  # load bot_config_wiki
            bot_config_wiki = locals()['bot_config_wiki']  # py3 compatibility
            self._flagenable = bot_config_wiki['flagenable']
            pywikibot.output(
                'Imported config %s rev %s from %s'
                % ((self._ConfCSSconfigPage.title(asLink=True), ) +
                   self._ConfCSSconfigPage.getVersionHistory(revCount=1)[0][:2]))

    def run(self, sim=False, msg=None):
        """Run SubsterBot()."""
        pywikibot.output(
        ...
        for page in self.generator:
            ...
            if sim:
                ...
                pywikibot.output('Getting page "{0}" from {1}...'
                                 .format(page.title(asLink=True), self.site))
            ...
            if sim:
                ...
                if substed_content != content:
                    pywikibot.showDiff(content, substed_content)
    ...
    def subContent(self, content, params):
        ...
        @type params: dict
        @return: Return a tuple containig the new content with tags
        ...
        for item in params:
            ...
            try:
                ...
                result = ''.join(traceback.format_exception(exc_info[0],
                                                            exc_info[1],
                                                            exc_info[2]))
                ...
                metadata['bot-error-traceback'] = self._bot_config[
                    'ErrorTemplate'] % (
                        item['value'],
                        pywikibot.Timestamp.now().isoformat(' '),
                        result.strip())

                # VerboseMode: IFF no 'bot-error-traceback' metadata tag
                # present on page, append it in order not to loose error info
                # (single exception)
            ...
            for data in metadata:
                ...
                (substed_content, tags) = self.subTag(substed_content,
                                                      value, metadata[data], 0)
                substed_tags += tags
    ...
    def subTemplate(self, content, param):
        ...
        # 0.5.) check cron/date
        if param['cron']:
            if isinstance(crontab, ImportError):
                pywikibot.warning('crontab package is not installed.')
        ...
        if param['url'][:7] == u'wiki://':
            ...
            f_url = http.fetch(uri=param['url'])
            external_buffer = f_url.content
            headers = f_url.response_headers
            if ('text/' not in headers['content-type']):
                pywikibot.output(u'Source is of non-text content-type, '
                                 u'using raw data instead.')
                external_buffer = f_url.raw
        ...
        if not ast.literal_eval(param['beautifulsoup']):    # DRTRIGON-88
            ...
            if external_data:    # not None
                ...
                if regex.groupindex:
                    for item in regex.groupindex:
                        external_data_dict[
                            '{0}-{1}'.format(param['value'], item)
                            ] = external_data[regex.groupindex[item] - 1]
                elif (len(external_data) == 1):
                    external_data_dict = {param['value']: external_data[0]}
                else:
                    external_data_dict = {param['value']: str(external_data)}
            pywikibot.debug(str(external_data_dict), _logger)

            param['postproc'] = eval(param['postproc'])
            # should be secured as given below, needs code changes in wiki too
            ...
            for value in external_data_dict:
                ...
                pywikibot.debug(external_data, _logger)
            ...
            if isinstance(BeautifulSoup, ImportError):
                pywikibot.warning('BeautifulSoup package is not installed.')
            else:
    ...
    def data_convertContent(self, substed_content):
        ...
        @return: Returns the extracted and converted data.
        """
    ...
    def data_save(self, page, data):
        ...
        Stores the changed content to linked labels.
        ...
        links = [{u'aliases': [u'%s:%s' % (dataitem, p.sortkeyprefix)],
                 ...
                 for p in pywikibot.Category(self.site, dataitem).articles()]
        ...
        for element in links:
            ...
            if (not claim) or (claim[0]['m'][3] != data[item]):
                ...
                timestr = '+0000000{0}Z'.format(
                    pywikibot.Timestamp.now().isoformat().split('.')[0])
                idstr = 'p{0}'.format(propid)
                try:
    ...
    def get_var_regex(self, var, cont='.*?'):
        ...
        @type  cont: string
        @return: Return the according (and compiled) regex object.
        """
        return re.compile(
    ...
    def xlsx2csv(self, external_buffer, sheet):
        ...
        if isinstance(excel, ImportError):
            pywikibot.warnig('openpyxl package is not installed')
        else:
        ...
        try:
            ...
            for sheet in doc.getElementsByType(odf.table.Table):
                ...
                for row in sheet.getElementsByType(odf.table.TableRow):
                    spamWriter.writerow(
                        [odf.teletype.extractText(cell).encode('utf-8')
                         for cell in
...
class SubsterMailbox(mailbox.mbox):
    ...
    def remove_duplicates(self):
        ...
        if remove:
            pywikibot.output(
                'Removed %i depreciated email data source(s).' % len(remove))
...
def main(*args):
    ...
    options['site'] = site = pywikibot.Site()
    # for several user's, but what about complete automation
    # (continous running...)
    for arg in local_args:
        if '-page' in arg[:5]:
            options['generator'] = [pywikibot.Page(site, arg[6:])]
        else:
            pywikibot.showHelp()
            return

    bot = SubsterBot(**options)
    try:
        bot.run()
