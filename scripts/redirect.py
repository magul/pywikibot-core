...
-sdtemplate:x  Add the speedy deletion template string including brackets.
               This enables overriting the default template via i18n or
               to enable speedy deletion for other projects than wikipedias.
...
class RedirectRobot(Bot):
    ...
    def get_sd_template(self):
        ...
        if self.getOption('delete') and not self.site.logged_in(sysop=True):
            ...
            if not sd and i18n.twhas_key(
                self.site, 'redirect-broken-redirect-template'):
            ...
            if sd:
                template = re.findall('.*?{{(.*?)[|}]', sd)
                title = template[0] if template else None
                if title:
                    page = pywikibot.Page(self.site, title, ns=10)
                    if page.exists():
                        return sd
            pywikibot.warning(
                'No speedy deletion template "{0}" available'.format(title))
        return None
