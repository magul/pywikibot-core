...
import pywikibot
from pywikibot import i18n, isbn_lib, pagegenerators, textlib, Bot, WikidataBot
from pywikibot.isbn_lib import InvalidIsbnException
from pywikibot.isbn_lib import ISBN, ISBN10, ISBN13
from pywikibot.tools import deprecated

# pyflakes workaround
__unused__ = (ISBN, ISBN10, ISBN13)
...
ranges = isbn_lib.RangeMessage().ranges  # held for compatibility only


@deprecated('isbn_lib.validate')
def is_valid(isbn):
    """Check whether an ISBN 10 or 13 is valid."""
    return isbn_lib.validate(isbn)


@deprecated('isbn_lib.get_isbn')
def getIsbn(code):
    """Return an ISBN object for the code."""
    return isbn_lib.get_isbn(code)
...
def _isbn10toIsbn13(match):
    ...
    i13 = isbn_lib.get_isbn(isbn).toISBN13()
    return i13.code
