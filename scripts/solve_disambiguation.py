    ...
    def getDisambiguationLinks(self, disambPage):
        ...
        if site_disamb_templates:
            ...
            for template in site_disamb_templates:
                template_regex = re.compile(r'\{\{ *(?:' + r':|'.join(self.site.namespaces[10]) + r':)?' + template + r' *(\}\}|\|)')
                stripped_text = textlib.replaceExcept(stripped_text, template_regex, r'', [])
...
or press enter to quit:""")
            ...
            try:
                if self.primary:
                    ...
                    try:
                        links = self.getDisambiguationLinks(disambPage)
                        links = [correctcap(l, disambPage.get())
                                 for l in links]
                    except pywikibot.NoPage:
