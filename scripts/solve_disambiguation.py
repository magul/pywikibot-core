...
class DisambiguationRobot(Bot):
    ...
    def removeDisambTemplateLinks(self, disambText, links):
        ...
        if site_disamb_templates:
            disamb_templates_regex = re.compile(
                r'\{\{ *(?:' + r':|'.join(self.site.namespaces[10]) + r':)?(' +
                r'|'.join(site_disamb_templates) + r') *[\}\|]'
            )
        ...
        if disambPage.isRedirectPage() and not self.primary:
            if (disambPage.site.lang in self.primary_redir_template and
                ...
                try:
                    ...
                    links = self.removeDisambTemplateLinks(disambPage2.text, links)
                    ...
                    links = self.removeDisambTemplateLinks(disambPage.text, links)
