...
class BasicBot(
    MultipleSitesBot,  # A bot only working on multiple sites
    ...
    @ivar summary_key: Edit summary message key. The message that should be
        used is placed on /i18n subdirectory. The file containing
        these messages should have the same name as the caller script
        (i.e. ndashredir.py in this case). Use summary_key to set a default
        edit summary message.
    @type summary_key: str
    """

    summary_key = 'ndashredir-create'
    ...
    def treat_page(self):
        ...
        if redir.title() == origin:
            pywikibot.output('\x1B[3mNo need to process %s, skipping…\x1B[23m'
                             % redir.title())
            # suggest -reversed parameter
            if ('-' in origin) and not self.getOption('reversed'):
                pywikibot.output(
                    '\x1B[3mConsider using -reversed parameter '
                    'for this particular page\x1B[23m')
        else:
            # skip existing
            if redir.exists():
                pywikibot.output('\x1B[3m%s already exists, skipping…\x1B[23m'
                                 % redir.title())
            else:
                ...
                if self.user_confirm(
                    ...
                    # i18n summary from i18n subdirectory with summary_key
                    # as summary key.
