...
class DownloadDumpBot(Bot):
    ...
    def get_dump_name(self, db_name, typ, date_revision):
        ...
        if os.path.isdir(db_path):
            ...
            if date_revision == 'latest':
                ...
                dump_filepath = dump_filepath_template.format(
                    db_name, date_revision, db_name, date_revision, typ)
                if os.path.isfile(dump_filepath):
                    return dump_filepath
        return None
    ...
    def build_dump_file_url(wikiname, revision, download_filename):
        ...
        @type wikiname: str
        @param revision: A revision date, formatted as YYYYMMDD or `latest`.
        @type revision: str
    ...
    def run(self):
        ...
        if (self.getOption('revision') == 'latest' and
