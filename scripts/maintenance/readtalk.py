...
def readtalk(lang, family_name, sysop=False):
    """Read the talk page and output it."""
    site = pywikibot.Site(code=lang, fam=family_name)
    if sysop:
        user = pywikibot.User(site, config.sysopnames[family_name][lang])
    else:
        user = pywikibot.User(site, config.usernames[family_name][lang])
...
def main():
    ...
    if getall:
        ...
        for family_name in namedict.keys():
            for lang in namedict[family_name].keys():
                readtalk(lang, family_name, sysop)
    else:
