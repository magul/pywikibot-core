"""Check that the latest commit diff follows the guidelines.                   testing
...
def get_latest_patchset():
    ...
    output = check_output(
        ['git', 'diff', '-U0', 'head^'], universal_newlines=True)
...
def check_line_lengths():
    ...
    """
    found_long = False
    for patched_file in get_latest_patchset():
...
def main():
    """Run this script."""
    if not check_line_lengths():
        raise SystemExit('Error: diff-checker failed')
