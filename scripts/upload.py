...
def main(*args):
    ...
    for arg in pywikibot.handle_args(args):
        if arg:
            if arg == '-always':
                ...
                description_file = arg[len('-descfile:'):]
            elif url == u'':
                url = arg
            else:
    ...
    if description_file:
        if description:
            pywikibot.error('A file specifying the description was provided '
                            'via -descfile: at the same time as it was '
                            'provided via the command line. Maybe there are '
                            'invalid arguments? Here is the content of the '
                            'description defined by the CLI:\n'
                            + ' '.join(description))
            return False
