...
from pywikibot import i18n, pagegenerators, editor, config
...
templates = {
    ...
    'en': {
        ...
        '{{copyedit}}': {
            'msg': 'Robot-assisted tagging: the writing of this article needs to be' +
            ' [[Wikipedia:How to copy-edit|copyedited]] and improved.',
            'pos': 'top'},
        ...
        '{{wikify}}': {
            'msg': 'Robot-assisted tagging: this article needs to be wikified per the' +
...
class CleaningBot(ExistingPageBot):

    """Bot meant to facilitate customized cleaning of the page."""

    def __init__(self, questions, questionlist, generator):
        # The question asked
        self.question = u"""(multiple numbers delimited with ',')
...
Enter) OK
What is it? """
        super(CleaningBot, self).__init__(generator=generator)
        self.site = pywikibot.Site()
        self.questions = questions
        self.questionlist = questionlist
        self.question = self.questions + self.question
    ...
    def handle_bad_page(self, *values):
        ...
        if site.lang not in list(done.keys()):
            pywikibot.output('Lang is not localised for the project.\n')
            return
        for d in pywikibot.translate(site.lang, done):
        ...
        while not answered:
            ...
            if answer == 'd':
                ...
                return
            if answer == 'e':
                text = editor.TextEditor().edit(self.page.get())
                msg = pywikibot.input(u'Summary message:')
                self.page.text = text
                self.page.save(comment=msg)
            if answer == 'b':
                pywikibot.output(u'Blanking page [[%s]].' % self.page.title())
                try:
                    self.page.put('', comment=i18n.twtranslate(site.lang,
                                  'followlive-blanking' % {'content': self.content}))
                except pywikibot.EditConflict:
                    pywikibot.output('An edit conflict occured ! Automatically retrying')
            ...
            try:
                choices = answer.split(',')
                    ...
                    pywikibot.error('"%s" is not valid' % answer)
                    continue
            # test input
        ...
        for choice in choices:
            ...
            if site.lang not in list(templates.keys()):
                pywikibot.output('\nLang is not localised for the project. Terminating program.')
                return
            tpl = pywikibot.translate(site.lang,
                                      templates)[self.questionlist[answer]]
            if tpl['pos'] == 'top':
                pywikibot.output(u'prepending %s...' % self.questionlist[answer])
                self.content = self.questionlist[answer] + '\n' + self.content
            elif tpl['pos'] == 'bottom':
                pywikibot.output(u'appending %s...' % self.questionlist[answer])
                self.content += '\n' + self.questionlist[answer]
            else:
                pywikibot.error(
                    u'"pos" should be "top" or "bottom" for template '
                    u'%s. Contact a developer.' % self.questionlist[answer])
                sys.exit('Exiting')
            summary += tpl['msg'] + ' '
            pywikibot.output(u'Probably added %s' % self.questionlist[answer])
#        pywikibot.output(newcontent) bug #2986247
        self.put_current(self.content, comment=summary)
    ...
    def run(self):
        for (page, date, length, loggedIn, username,
                comment) in self.site.newpages(step=100):
...
def main(*args):
    ...
    if config.mylang not in list(templates.keys()):
        pywikibot.output('\nLang is not localised for the project. Terminating program.')
    else:
        ...
        bot = CleaningBot(questions, questionlist, generator)
        bot.run()
