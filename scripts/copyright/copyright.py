...
class URLExclusion:
    ...
    """Create backup and then exclude given page list urls."""
    ...
    def pages_list(self):
        for i in pages_for_exclusion_database:
            path = config.datafilepath(appdir, i[0], i[2])
            config.makepath(path)
        ...
        for page, path in self.pages_list():
            ...
            if download:
                ...
                try:
                    ...
                    pywikibot.error('Getting page failed')
    ...
    def scan(self):
        ...
        result_list += read_file(
            config.datafilepath(appdir, 'exclusion_list.txt'),
    ...
    def dump(self):
        with open(pywikibot.config.datafilepath(appdir, 'exclusion_list.dump'),
                  'w') as f:
            f.write('\n'.join(self.URLlist))
...
class SearchEngine:
    ...
    """Facilitate search engine functionality."""
            ...
            try:
                if engine == 'google':
                    ...
                    try:
                        ...
                        pywikibot.error('Live Search Error: %s' % err)
                        raise
...
class WebPage(object):
    ...
    """Enable operation on exising webpage."""
    ...
    def __init__(self, url, URLExcl):
        ...
        try:
            ...
            if err.code >= 400:
                ...
                raise NoWebPage
            return None
        except urllib2.URLError as arg:
    ...
    def check_in_source(self):
        """The function avoids errors in search results.
...
def get_by_id(title, id):
    return http.request(pywikibot.Site(),
                        "/w/index.php?title=%s&oldid=%s&action=raw" %
                        (title, id))
            ...
            if output:
                write_log(
                    ...
                    % (title.replace(' ', '_').replace("\"", "%22"),
                       id, 'author') + output,
                    config.datafilepath(appdir, 'ID_output.txt'))


class CheckRobot():

    """Bot responsible to check for copyright text on specified search engines."""
...
def put(page, text, comment):
    while True:
        try:
            ...
            break
        except pywikibot.SpamfilterError as url:
