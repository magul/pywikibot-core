...
class FrozenDict(dict):
    ...
    def pop(self, key):
        """Prevent updates but react the same if key is missing."""
        if key in self:
            raise TypeError(self._error)
        else:
            raise KeyError(key)
    ...
    def popitem(self):
        """Prevent updates but react the same if it is empty."""
        if self:
            raise TypeError(self._error)
        else:
            # raise the message generated by this versions dict
            return super(FrozenDict, self).popitem()

    def setdefault(self, key, default):
        """Prevent updates when the key is not in the dict."""
        if key in self:
            return self[key]
        else:
            raise TypeError(self._error)
