...
from pywikibot.tools import deprecated, deprecated_args, issue_deprecation_warning, StringTypes
...
def twtranslate(source, twtitle, parameters=None, fallback=True,
    ...
    @param source: When it's a site it's using the lang attribute and otherwise it
    ...
    # check whether we need the language code back
        ...
        warn('The source argument should not be a list but either a BaseSite or '
...
def twhas_key(source, twtitle):
    ...
    if hasattr(source, 'lang'):
        source = source.lang
    transdict = _get_translation(source, twtitle)
