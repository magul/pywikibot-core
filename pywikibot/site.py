        ...
        if self.is_oauth_token_available():
            if sysop:
                ...
                if self._username == [None, None]:
                    raise NoUsername('No username has been defined in your user-config.py: you '
                                     'have to add in this file the following line:\n'
                                     "usernames['{family}']['{lang}'] = '{username}'"
                                     .format(family=self.family, lang=self.lang,
                                             username=self.userinfo['name']))
                else:
                    raise NoUsername('Logged in on {site} via OAuth as {wrong}, but expect as '
                                     '{right}'.format(site=self, wrong=self.userinfo['name'],
