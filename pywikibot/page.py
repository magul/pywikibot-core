...
class Revision(DotReadableDict):
    ...
    def from_API(cls, rev, page):
        """Create a Revision from a rev and Page object."""
    ...
    @property
    def text(self):
        """Return and cache the text of the revision."""
        if not self._text:
            self._get_all()
        return self._text
    ...
    @property
    def comment(self):
        """Return the comment about the revision."""
    ...
    def rollbacktoken(self):
        """Return the rollback token for the revision."""
        if not self._token:
...
class _RevisionCache(UnicodeMixin):
    ...
    def __unicode__(self):
        return u'RevisionCache({0},[{1}])'.format(
            self._page.title(),
            '],['.join(','.join(str(rev.revid) for rev in chunk)
    ...
    @staticmethod
    def _bisect_right(a, x, key=lambda e: e, reverse=False):
        return _RevisionCache._keyed_func(False, a, x, key=key, reverse=reverse)

    @staticmethod
    def _bisect_left(a, x, key=lambda e: e, reverse=False):
    ...
    def _get_cached(cls, cached_block, endid, endtime, total, check):
        ...
        for index in range(until):
            if cached_block[index].revid == endid:
                ...
                break
        if endtime:
            until = min(until, cls._bisect_right(short_block, endtime,
                                                 key=cls._key_time, reverse=None))
    ...
    def _revision_chunk_generator(self, content=False, rollback=False,
        ...
        for page_data in gen:
            if not self._page.site.sametitle(
                ...
                raise pywikibot.Error(
                    'Query on "{0}" returned data on "{1}"'.format(
    ...
    def get_revisions(self, content=False, startid=None, endid=None,
        ...
        if starttime is not None:
            ...
            chunk_index = self._bisect_right(self._cache, starttime,
            ...
            if (chunk_index < len(self._cache) and
                ...
                if reverse:
                    idx = self._bisect_right(self._cache[chunk_index], starttime,
                                             key=self._key_time, reverse=True)
                else:
                    idx = self._bisect_left(self._cache[chunk_index], starttime,
                                            key=self._key_time, reverse=True)
            ...
            if chunk_index >= len(self._cache):
                ...
                # print('startid entry #{0}C{1} id {2}'.format(
                #     chunk_index, idx, self._cache[chunk_index][idx].revid))
        ...
        while total is None or total > 0:
            if idx is not False and idx >= 0:
                ...
                # print('Idx: {0} ID: {1}'.format(idx, ','.join(str(_.revid)
                #                                 for _ in cached_block)))
                cached_block, stopped = self._get_cached(cached_block, endid, endtime,
                                                         total, check_after)
            ...
            for pagedata in gen:
                ...
                # print('Chunk: {0} in Chains: {1}'.format(
                #    [_.revid for _ in chunk], [[_.revid for _ in c] for c in chains]))
