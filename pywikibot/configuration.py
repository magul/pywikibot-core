...
class _Configuration(object):
    ...
    def __init__(self, base_dir):
        ...
        self.register_families_folder(os.path.join(os.path.dirname(__file__), 'families'))
...
class _ConfigurationWrapper(object):
    ...
    def _load(self, path):
        ...
        super(_ConfigurationWrapper, self).__setattr__('_configuration', _Configuration(path))
        ...
        for name, value in list(user_config.items()):
            if name.startswith('_'):
                ...
                if new_type == old_type or value is None or old_value is None:  # nopep8
                    pass
                elif new_type is int and (old_type is float or old_type is bool):
                    pass
                elif old_type is int and (new_type is float or new_type is bool):
                    pass
        ...
        for name in _Configuration.__dict__:
            if name[0] == '_':
                continue
            # TODO: Originally it was always copying usernames, sysopnames, disambiguation_comment
        ...
        if self._configuration.transliteration_target == 'not set':
            if sys.platform == 'win32':
                self._configuration.transliteration_target = self._configuration.console_encoding
        ...
        if name[0] != '_':
            if not type(value) in [types.FunctionType, types.ModuleType]:
                if show_all or name in _modified:
                    if name in configuration._configuration._private_values and value:
