...
from pywikibot.comms import default_socket_timeout
...
class EventStreams(object):
    ...
    def __init__(self, streamtype='recentchange', site=None, **kwargs):
        """Constructor.

        @param streamtype: event streamtype, default is 'recentchange'
            Used when no url is given.
        @type streamtype: str
        @param site: a project site object. Used when no url is given
        @type site: APISite
        ...
        kwargs.setdefault('timeout', default_socket_timeout())
        ...
        if not hasattr(self, '_url'):
            if self._streamtype is None:
                raise NotImplementedError(
                    'No streamtype specified for class {0}'
    ...
    def register_filter(self, *args, **kwargs):
        ...
        (see below). If no 'ftype' keyword argument is given, 'all' is assumed
        as default.
        ...
        Filter functions
        ================
        Filter may be given as external function methods given as positional
        argument like::
        ...
        The data dict from event is passed to the external filter function
        as a parameter and that method must handle it in a proper way and
        return True if the filter matches and False otherwise.

        Filter keys and values
        ======================
        Another method to register a filter is to pass pair of keys and values
        to as keyword arguments to this method. The key must be a key of the
        event data dict and the value must be any value or an iterable of
        values the C{data['key']} may match or be part of it. Samples::
        ...
        @type args: callable
        @param kwargs: any key returned by eventstreams with a value
            returned by eventstreams for a given key
        @type kwargs: str or list or tuple or other sequence
        @raise TypeError: when given args parameter is not a callable.
        """
    ...
    def __iter__(self):
        """Iterator."""
        n = 0
        for event in EventSource(**self.sse_kwargs):
