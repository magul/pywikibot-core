...
class EventStreams(object):
    ...
    def register_filter(self, ftype='all', **kwargs):
        """Register a filter.

        @param ftype: The filter type, one of 'all', 'any', 'none'
        @type ftype: str
        @keyword <anykey>: any key returned by eventstreams with a value
            returned by eventstreams for a given key
        @type <anykey>: str or list or tuple or other sequence
        """
