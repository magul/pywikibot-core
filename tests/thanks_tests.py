...
class TestThankRevision(TestCase):
    ...
    def test_thank_revision(self):
        ...
        log_entries = site.logevents(logtype='thanks', total=5, start=before_time,
                                     page=user, reverse=True)
    ...
    def test_bad_recipient(self):
        ...
        if can_thank:
            self.skipTest('There is no recent change which can be test thanked.')
        self.assertRaises(APIError, Revision._thank, revid, site, source='pywikibot test')
