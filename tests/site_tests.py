...
class TestSiteGenerators(DefaultSiteTestCase):
    ...
    def test_categorymembers(self):
        cats = self.site.pagecategories(self.mainpage)
        if len(cats) > 0:
            cat = cats[0]
            for cm in self.site.categorymembers(cat):
                self.assertIsInstance(cat, pywikibot.Page)

    def test_pageimages(self):
        for im in self.site.pageimages(self.mainpage):
            self.assertIsInstance(im, im, pywikibot.FilePage)
    ...
    def test_pagelinks(self):
        ...
        for target in self.site.preloadpages(self.site.pagelinks(self.mainpage,
