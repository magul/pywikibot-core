...
class TestSiteLoadRevisions(TestCase):
    ...
    def testLoadRevisions_basic(self):
        ...
        self.assertIsNone(self.mainpage._revision_cache[self.mainpage._revid].text)
    ...
    def testLoadRevisions_getText(self):
        ...
        self.assertFalse(hasattr(self.mainpage, '_text'))
        self.assertIn(self.mainpage._revid, self.mainpage._revision_cache)
        self.assertIsNotNone(self.mainpage._revision_cache[self.mainpage._revid].text)
        self.assertTrue(self.mainpage._revision_cache[self.mainpage._revid].text)
        self.assertIsNotNone(self.mainpage.text)
...
class TestPagePreloading(DefaultSiteTestCase):
    ...
    def test_preload_langlinks_count(self):
        ...
        for page in pages:
            ...
            self.assertIsInstance(page.exists(), bool)
            if page.exists():
                self.assertEqual(len(page._revision_cache), 1)
                self.assertTrue(hasattr(page, "_text"))
                self.assertFalse(hasattr(page, '_pageprops'))
            count += 1
