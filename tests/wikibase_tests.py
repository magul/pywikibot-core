...
class TestItemLoad(WikidataTestCase):
    ...
    4. ItemPage.from_entity_url(url, site)
    ...
    def test_from_entity_url(self):
        """Test ItemPage.from_entity_url."""
        repo = self.get_repo()
        entiry_url = 'http://www.wikidata.org/entity/Q124'
        self.assertEqual(ItemPage.from_entity_url(entiry_url, repo),
                         ItemPage(repo, 'Q124'))

    def test_from_entity_url_not_a_data_repo(self):
        """Test ItemPage.from_entity_url with a non-Wikibase site."""
        repo = self.site
        entiry_url = 'http://www.wikidata.org/entity/Q124'
        self.assertRaises(TypeError,
                          ItemPage.from_entity_url, entiry_url, repo)

    def test_from_entity_url_wrong_repo(self):
        """Test ItemPage.from_entity_url with unexpected item repo."""
        repo = self.get_repo()
        entiry_url = 'http://test.wikidata.org/entity/Q124'
        self.assertRaises(ValueError,
                          ItemPage.from_entity_url, entiry_url, repo)

    def test_from_entity_url_invalid_title(self):
        """Test ItemPage.from_entity_url with an invalid item title format."""
        repo = self.get_repo()
        entiry_url = 'http://www.wikidata.org/entity/Nonsense'
        self.assertRaises(pywikibot.InvalidTitle,
                          ItemPage.from_entity_url, entiry_url, repo)

    def test_from_entity_url_no_item(self):
        """Test ItemPage.from_entity_url with non-exitent item."""
        repo = self.get_repo()
        entiry_url = 'http://www.wikidata.org/entity/Q999999999999999999'
        self.assertRaises(pywikibot.NoPage,
                          ItemPage.from_entity_url, entiry_url, repo)

    def test_from_entity_url_no_item_lazy(self):
        """Test ItemPage.from_entity_url with lazy loaded non-exitent item."""
        repo = self.get_repo()
        entiry_url = 'http://www.wikidata.org/entity/Q999999999999999999'
        expected_item = ItemPage(repo, 'Q999999999999999999')
        self.assertEqual(
            ItemPage.from_entity_url(entiry_url, repo, lazy_load=True),
            expected_item)

        self.assertFalse(expected_item.exists())  # enure actually missing
