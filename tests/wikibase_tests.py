...
class TestGeneral(WikidataTestCase):
    ...
    def testWikibase(self):
        ...
        regex = '.* is not type .*\.'
        self.assertRaisesRegex(ValueError, regex, claim.setTarget, value="test")
...
class TestWikibaseCoordinate(WbRepresentationTestCase):
    ...
    def test_Coordinate_dim(self):
        ...
        regex = 'No values set for dim or precision'
...
class TestWbTime(WbRepresentationTestCase):
    ...
    def test_WbTime_timestamp(self):
        ...
        t = pywikibot.WbTime(site=repo, year=-2010, hour=12, minute=43)
        regex = 'You cannot turn BC dates into a Timestamp'
    ...
    def test_WbTime_errors(self):
        ...
        repo = self.get_repo()
        regex = 'no year given'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTime, site=repo,
                               precision=15)
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTime, site=repo,
...
class TestWbQuantity(WbRepresentationTestCase):
    ...
    def test_WbQuantity_errors(self):
        ...
        regex = 'no amount given'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbQuantity,
                               amount=None, error=1)
...
class TestWbMonolingualText(WbRepresentationTestCase):
    ...
    def test_WbMonolingualText_errors(self):
        ...
        regex = 'text and language cannot be empty'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbMonolingualText,
                               text='', language='sv')
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbMonolingualText,
...
class TestWbGeoShapeNonDry(WbRepresentationTestCase):
    ...
    def test_WbGeoShape_error_on_non_page(self):
        ...
        regex = 'Page must be a pywikibot\.Page object\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbGeoShape,
    ...
    def test_WbGeoShape_error_on_non_exitant_page(self):
        ...
        regex = 'Page must exist\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbGeoShape, page,
                               self.get_repo())
    ...
    def test_WbGeoShape_error_on_wrong_site(self):
        ...
        repo = self.get_repo()
        page = Page(repo, 'Q123')
        regex = 'Page must be on the geo-shape repository site\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbGeoShape, page,
                               self.get_repo())
    ...
    def test_WbGeoShape_error_on_wrong_page_type(self):
        ...
        regex = "Page must be in 'Data:' namespace and end in '\.map' for geo-shape\."
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbGeoShape,
...
class TestWbTabularDataNonDry(WbRepresentationTestCase):
    ...
    def test_WbTabularData_error_on_non_page(self):
        ...
        regex = 'Page must be a pywikibot\.Page object\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTabularData,
                               'A string', self.get_repo())
    ...
    def test_WbTabularData_error_on_non_exitant_page(self):
        ...
        page = Page(self.commons, 'Non-existant page... really')
        regex = 'Page must exist\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTabularData, page,
                               self.get_repo())
    ...
    def test_WbTabularData_error_on_wrong_site(self):
        ...
        repo = self.get_repo()
        page = Page(repo, 'Q123')
        regex = 'Page must be on the tabular-data repository site\.'
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTabularData, page,
                               self.get_repo())
    ...
    def test_WbTabularData_error_on_wrong_page_type(self):
        ...
        regex = "Page must be in 'Data:' namespace and end in '\.tab' for tabular-data\."
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTabularData,
                               non_data_page, self.get_repo())
        self.assertRaisesRegex(ValueError, regex, pywikibot.WbTabularData,
                               non_map_page, self.get_repo())
...
class TestItemLoad(WikidataTestCase):
    ...
    def test_item_invalid_titles(self):
        ...
        regex = "'.*' is not a valid .* page title"
        for title in ['null', 'NULL', 'None',
                      '-2', '1', '0', '+1', 'Q0',
                      'Q0.5', 'Q', 'Q-1', 'Q+1']:
            self.assertRaisesRegex(pywikibot.InvalidTitle, regex, ItemPage,
                                   wikidata, title)
        
        regex = "Item's title cannot be empty"
        self.assertRaisesRegex(pywikibot.InvalidTitle, regex, ItemPage,
                               wikidata, '')
    ...
    def test_fromPage_invalid_title(self):
        ...
        regex = '.* contains illegal char\(s\) .*'
        self.assertRaisesRegex(pywikibot.InvalidTitle, regex, ItemPage.fromPage,
                               page)
        ...
        for props in [True, False]:
            for method in ['title', 'get', 'getID', 'exists']:
                ...
                if method == 'exists':
                    ...
                    regex = "Page .* doesn't exist\."
                    self.assertRaisesRegex(pywikibot.NoPage, regex,
                                           getattr(item, method))
    ...
    def test_fromPage_missing_lazy(self):
        ...
        self.assertRaisesRegex(pywikibot.NoPage, 'Test page', ItemPage.fromPage,
    ...
    def test_from_entity_uri_not_a_data_repo(self):
        ...
        repo = self.site
        entity_uri = 'http://www.wikidata.org/entity/Q124'
        regex = '.* is not a data repository\.'
        self.assertRaisesRegex(TypeError, regex, ItemPage.from_entity_uri, repo,
                               entity_uri)
    ...
    def test_from_entity_uri_wrong_repo(self):
        ...
        regex = 'The supplied data repository \(.*\) does not correspond to that of the item \(.*\)'
        self.assertRaisesRegex(ValueError, regex, ItemPage.from_entity_uri,
                               repo, entity_uri)
    ...
    def test_from_entity_uri_invalid_title(self):
        ...
        regex = "'.*' is not a valid .* page title"
        self.assertRaisesRegex(pywikibot.InvalidTitle, regex,
...
class TestRedirects(WikidataTestCase):
    ...
    def test_normal_item(self):
        ...
        self.assertTrue(item.exists())
        regex = 'Page .* is not a redirect page\.'
...
class TestPropertyPage(WikidataTestCase):
    ...
    def test_property_empty_property(self):
        ...
        wikidata = self.get_repo()
        regex = "Property's title cannot be empty"
        self.assertRaisesRegex(pywikibot.InvalidTitle, regex, PropertyPage,
                               wikidata)
...
class TestClaimSetValue(WikidataTestCase):
    ...
    def test_set_incorrect_target_value(self):
        ...
        regex = '.* is not type .*\.'
        self.assertRaisesRegex(ValueError, regex, date_claim.setTarget, 'foo')
        url_claim = pywikibot.Claim(wikidata, 'P856')
        self.assertRaisesRegex(ValueError, regex, url_claim.setTarget, pywikibot.WbTime(2001, site=wikidata))
        mono_claim = pywikibot.Claim(wikidata, 'P1450')
        self.assertRaisesRegex(ValueError, regex, mono_claim.setTarget, 'foo')
        quantity_claim = pywikibot.Claim(wikidata, 'P1106')
        self.assertRaisesRegex(ValueError, regex, quantity_claim.setTarget, 'foo')
...
class TestDryPageGetNotImplemented(DefaultWikibaseClientTestCase,
    ...
    def test_base_get_args(self):
        ...
        item._content = {}
        self.assertRaises(NotImplementedError, item.get, force=True, sysop=True)
        self.assertRaises(NotImplementedError, item.get, force=False,
                          sysop=True)
        self.assertRaises(NotImplementedError, item.get, force=False,
                          sysop=False)
...
class TestNamespaces(WikidataTestCase):
    ...
    def test_empty_wikibase_page(self):
        ...
        regex = 'Namespace ".*" is not valid for Wikibase entity type ".*"'
        self.assertRaisesRegex(ValueError, regex, WikibasePage, wikidata, ns=0,
                               entity_type='property')
        self.assertRaisesRegex(ValueError, regex, WikibasePage, wikidata,
    ...
    def test_wrong_namespaces(self):
        ...
        regex = '.*: Namespace ".*" is not valid'
        self.assertRaisesRegex(ValueError, regex, WikibasePage, wikidata,
                               title='Wikidata:Main Page')
        regex = "'.*' is not in the namespace .*"
        self.assertRaisesRegex(ValueError, regex, ItemPage, wikidata, 'File:Q1')
        self.assertRaisesRegex(ValueError, regex, PropertyPage, wikidata,
                               'File:P60')
    ...
    def test_item_unknown_namespace(self):
        ...
        #       raise an error.
        wikidata = self.get_repo()
        regex = "'.*' is not a valid item page title"
        self.assertRaisesRegex(pywikibot.InvalidTitle, regex, ItemPage,
                               wikidata, 'Invalid:Q1')
...
class TestOwnClient(TestCase):
    ...
    def test_page_from_repository_fails(self, key):
        ...
        site = self.get_site(key)
        dummy_item = 'Q1'
        regex = 'page_from_repository method is not implemented for Wikibase .*\.'
        self.assertRaisesRegex(NotImplementedError, regex,
...
class TestUnconnectedClient(TestCase):
    ...
    def test_page_from_repository_fails(self, key):
        ...
        site = self.get_site(key)
        dummy_item = 'Q1'
        regex = 'Wikibase is not implemented for .*\.'
        self.assertRaisesRegex(pywikibot.UnknownExtension, regex,
                          site.page_from_repository, dummy_item)
