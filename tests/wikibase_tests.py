...
class TestWikibaseGetResponse(WikidataTestCase):

    """Test the members of _contents are all known."""

    cached = True

    def _check_contents(self, entity):
        """Check the contents of the entity."""
        nested_data = dict(
            (key, value) for key, value in entity._content.items()
            if isinstance(value, dict) or isinstance(value, list))

        non_nested_data = dict(
            (key, value) for key, value in entity._content.items()
            if key not in nested_data)

        self.assertEqual(set(non_nested_data.keys()), entity._metadata_keys)
        self.assertGreaterEqual(entity._content_keys, set(nested_data.keys()))

    def test_normal_item(self):
        """Test normal item _content."""
        item = ItemPage(self.repo, 'Q60')
        item.get()
        self._check_contents(item)

    def test_redirect(self):
        """Test redirect _content."""
        redirect = ItemPage(self.repo, 'Q10008448')
        redirect.get(get_redirect=True)
        self._check_contents(redirect)

    def test_redirect_target(self):
        """Test redirect target _content."""
        redirect = ItemPage(self.repo, 'Q10008448')
        redirect.get(get_redirect=True)
        target = redirect.getRedirectTarget()
        target.get()
        self._check_contents(target)

    def test_property(self):
        """Test property _content."""
        property_page = PropertyPage(self.repo, 'P31')
        property_page.get()
        self._check_contents(property_page)
