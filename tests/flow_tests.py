...
class TestFlowFactoryErrors(TestCase):
    ...
    def test_illegal_arguments(self):
        ...
        self.assertRaisesRegex(TypeError,
            pywiki_flow_object_error, Topic.from_topiclist_data, self.site, '', {})
        self.assertRaisesRegex(TypeError,
                               topic_uuid_string_error, Topic.from_topiclist_data, board, 521, {})
        self.assertRaisesRegex(TypeError,
                               illegal_post_data_error, Topic.from_topiclist_data, board,
                          'slbktgav46omarsd', [0, 1, 2])
        self.assertRaisesRegex(NoPage,
                               topic_must_exist_error, Topic.from_topiclist_data, board,
                          'abc', {'stuff': 'blah'})

        # Post.fromJSON
        self.assertRaisesRegex(TypeError,
                               page_topic_object_error, Post.fromJSON, board, 'abc', {})
        self.assertRaisesRegex(TypeError,
                               post_uuid_string_error, Post.fromJSON, real_topic, 1234, {})
        self.assertRaisesRegex(TypeError,
                               illegal_post_data_error, Post.fromJSON, real_topic, 'abc', [])
        self.assertRaisesRegex(NoPage,
                               topic_must_exist_error, Post.fromJSON, fake_topic, 'abc',
                          {'posts': [], 'revisions': []})
    ...
    def test_invalid_data(self):
        ...
        self.assertRaisesRegex(ValueError,
                          ...
                          {'posts': [], 'revisions': []})
        self.assertRaisesRegex(ValueError,
                               current_post_revision_error, Topic.from_topiclist_data, board,
                          'slbktgav46omarsd',
        ...
        self.assertRaisesRegex(ValueError,
                          ...
                          {'stuff': 'blah'})
        self.assertRaisesRegex(ValueError,
                               current_post_revision_error, Post.fromJSON, real_topic, 'abc',
                          {'posts': {'abc': ['123']},
                           'revisions': {'456': []}})
        self.assertRaisesRegex(AssertionError,
