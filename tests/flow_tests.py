...
current_post_revision_error = 'Current revision of postnot found in supplied data.*'
...
class TestFlowFactoryErrors(TestCase):
    ...
    def test_illegal_arguments(self):
        ...
        self.assertRaisesRegex(TypeError, pywiki_flow_object_error, Topic.from_topiclist_data, self.site, '', {})
        self.assertRaisesRegex(TypeError, topic_uuid_string_error, Topic.from_topiclist_data, board, 521, {})
        self.assertRaisesRegex(TypeError, illegal_post_data_error, Topic.from_topiclist_data, board,
                          'slbktgav46omarsd', [0, 1, 2])
        self.assertRaisesRegex(NoPage, topic_must_exist_error, Topic.from_topiclist_data, board,
                          'abc', {'stuff': 'blah'})

        # Post.fromJSON
        self.assertRaisesRegex(TypeError, page_topic_object_error, Post.fromJSON, board, 'abc', {})
        self.assertRaisesRegex(TypeError, post_uuid_string_error, Post.fromJSON, real_topic, 1234, {})
        self.assertRaisesRegex(TypeError, illegal_post_data_error, Post.fromJSON, real_topic, 'abc', [])
        self.assertRaisesRegex(NoPage, topic_must_exist_error, Post.fromJSON, fake_topic, 'abc',
                          {'posts': [], 'revisions': []})
    ...
    def test_invalid_data(self):
        ...
        real_topic = Topic(self.site, 'Topic:Slbktgav46omarsd')
        # Topic.from_topiclist_data
        self.assertRaisesRegex(ValueError, illegal_post_data_error, Topic.from_topiclist_data,
                          board, 'slbktgav46omarsd', {'stuff': 'blah'})
        self.assertRaisesRegex(ValueError, post_not_found_error, Topic.from_topiclist_data,
                          board, 'slbktgav46omarsd',
                          {'posts': [], 'revisions': []})
        self.assertRaisesRegex(ValueError, current_post_revision_error, Topic.from_topiclist_data, board,
                          'slbktgav46omarsd',
                          {'posts': {'slbktgav46omarsd': ['123']},
                           'revisions': {'456': []}})
        self.assertRaisesRegex(AssertionError, '', Topic.from_topiclist_data, board,
                          'slbktgav46omarsd',
                          {'posts': {'slbktgav46omarsd': ['123']},
                           'revisions': {'123': {'content': 789}}})

        # Post.fromJSON
        self.assertRaisesRegex(ValueError, illegal_post_data_error, Post.fromJSON, real_topic, 'abc', {})
        self.assertRaisesRegex(ValueError, illegal_post_data_error, Post.fromJSON, real_topic, 'abc',
                          {'stuff': 'blah'})
        self.assertRaisesRegex(ValueError, current_post_revision_error, Post.fromJSON, real_topic, 'abc',
                          {'posts': {'abc': ['123']},
                           'revisions': {'456': []}})
        self.assertRaisesRegex(AssertionError, '', Post.fromJSON, real_topic, 'abc',
                          {'posts': {'abc': ['123']},
                           'revisions': {'123': {'content': 789}}})
