...
# BotPasswords. See user-config.py for general informations about user- files.
#
...
def create_user_config(args=None, force=False):
            ...
            if pywikibot.input_yn('Do you want to add a BotPassword for {0}?'
                ...
                if not botpasswords:
                    ...
                    pywikibot.output('Please note that plain text in {0} and '
                                     'anyone with read access to that directory'
                                     'will be able read the file.'
                                     .format(_fncpass))
    ...
    if botpasswords:
        try:
            # Firest create an empty file with good permissions, before writing
            # in it
